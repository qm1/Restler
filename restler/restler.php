...
class Restler
{
    ...
    protected function removeMutltiSlashesDirMapping($array) {
        $parts = array();
        $i = 0;
    ...
    */
    protected function getPath()
    {
        ...
        if (empty($path)) {
            ...
            $dir = $parts[$num_parts - 2];
            
            $parts = $this->removeMutltiSlashesDirMapping(explode('/', $_SERVER['SCRIPT_NAME']));
            $full_path = implode('/', $parts);
            // remove from the $full_path everything up to and including the $dir
            $parts = explode($dir . '/', $full_path);
            $full_path = $parts[count($parts) - 1];
            return $full_path;
        }
        return $path;
    }
        ...
        foreach ($methods as $method) {
            ...
            if (preg_match_all(
                ...
                foreach ($params as $param) {
                    ...
                    if ($http_method == 'PUT' && $num_put_arg++ > 0) { // only one URL argument is supported for PUT
                        break;
                    }
                    $url .= $url == '' ? ':' : '/:';
                    $url .= $param->getName();
